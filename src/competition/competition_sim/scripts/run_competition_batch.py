...
def print_case_result(index, total, case):
    ...
    print(
        "[%d/%d] %s %s duration=%s clearance=%s drop=%s"
        % (
            index,
            total,
            status,
...
def main():
    ...
    if args.ros_port_base + total - 1 > 65535:
        raise SystemExit("ROS port range exceeds 65535")
                ...
                cases.append(
                    {
                        ...
                        "ros_port": args.ros_port_base + case_index - 1,
